...
def send_some(msgType, mediaId, additionalDict={}, targetIdList=[]):
    ...
    return _send_some(msgType, mediaId, additionalDict, targetIdList)

def send_all(msgType, mediaId, additionalDict={}, tagId=None):
    @retry(n=3, waitTime=3)
    @access_token
    def _send_all(msgType, mediaId, additionalDict, tagId, accessToken=None):
        msgDict = __form_send_dict(msgType, mediaId, additionalDict)
        if not msgDict: return msgDict
        if tagId is None: 
        ...
        try:
            ...
            return ReturnValue({'errcode': -10001, 'errmsg': e.message})
    return _send_all(msgType, mediaId, additionalDict, tagId)

def preview(msgType, mediaId, additionalDict={}, toUserId=None, toWxAccount=None):
    @retry(n=3, waitTime=3)
    @access_token
    def _preview(msgType, mediaId, additionalDict,
            toUserId, toWxAccount, accessToken=None):
        msgDict = __form_send_dict(msgType, mediaId, additionalDict)
        if not msgDict: return msgDict
        try:
    ...
    return _preview(msgType, mediaId, additionalDict, toUserId, toWxAccount)
...
def delete(msgId):
    @retry(n=3, waitTime=3)
    @access_token
    def _delete(msgId, accessToken=None):
        try:
            ...
            return ReturnValue({'errcode': -10001, 'errmsg': e.message})
    return _delete(msgId)

def get(msgId):
    @retry(n=3, waitTime=3)
    @access_token
    def _get(msgId, accessToken=None):
        try:
...
def upload(fileType, openedFile, additionalDict={}, perment=False):
    @retry(n=3, waitTime=3)
    @access_token
    def _upload(fileType, openedFile, additionalDict, perment, accessToken=None):
        ...
        try:
            ...
            return ReturnValue({'errcode': -10001, 'errmsg': e.message})
    return _upload(fileType, openedFile, additionalDict, perment)

def download(mediaId):
    @retry(n=3, waitTime=3)
    @access_token
    def _download(mediaId, accessToken=None):
        try:
            ...
            return ReturnValue({'errcode': -10001, 'errmsg': e.message})
    return _download(mediaId)

def get_material(mediaId):
    @retry(n=3, waitTime=3)
    @access_token
    def _get_material(mediaId, accessToken=None):
        try:
            ...
            return ReturnValue({'errcode': -10001, 'errmsg': e.message})
    return _get_material(mediaId)

def delete_material(mediaId):
    @retry(n=3, waitTime=3)
    @access_token
    def _delete_material(mediaId, accessToken=None):
        try:
            ...
            return ReturnValue({'errcode': -10001, 'errmsg': e.message})
    return _delete_material(mediaId)

def get_materialcount(mediaId):
    @retry(n=3, waitTime=3)
    @access_token
    def _get_materialcount(mediaId, accessToken=None):
        try:
            ...
            return ReturnValue({'errcode': -10001, 'errmsg': e.message})
    return _get_materialcount(mediaId)
...
def batchget_material(fileType, offset=0, count=20):
    ...
    def _batchget_material(fileType, offset, count, accessToken=None):
        ...
        try:
            ...
            return ReturnValue({'errcode': -10001, 'errmsg': e.message})
    return _batchget_material(fileType, offset, count)

def create_news(newsDict, perment=False):
    @retry(n=3, waitTime=3)
    @access_token
    def _create_news(newsDict, perment, accessToken=None):
        if perment:
        ...
        try:
            ...
            return ReturnValue({'errcode': -10001, 'errmsg': e.message})
    return _create_news(newsDict, perment)

def update_news(newsDict):
    @retry(n=3, waitTime=3)
    @access_token
    def _update_news(newsDict, accessToken=None):
        try:
            ...
            return ReturnValue({'errcode': -10001, 'errmsg': e.message})
    return _update_news(newsDict)

def get_image_url(openedFile):
    @retry(n=3, waitTime=3)
    @access_token
    def _get_image_url(openedFile, accessToken=None):
        try:
            ...
            return ReturnValue({'errcode': -10001, 'errmsg': e.message})
    return _get_image_url(openedFile)
