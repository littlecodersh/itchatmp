...
def set_head_image(openedFile, kfAccount):
    @retry(n=3, waitTime=3)
    @access_token
    def _set_head_image(openedFile, kfAccount, accessToken=None):
        try:
    ...
    return _set_head_image(openedFile, kfAccount)
...
def send(msgType, mediaId, additionalDict={}, toUserId=''):
    ...
    def _send(msgType, mediaId, additionalDict, toUserId, accessToken=None):
        ...
        try:
            ...
            return ReturnValue({'errcode': -10001, 'errmsg': e.message})
    return _send(msgType, mediaId, additionalDict, toUserId)
